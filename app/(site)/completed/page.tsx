    ...
    const fetchCompletedProjects = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_PAYLOAD_URL || "http://localhost:3000"}/api/completed?locale=${i18n.language}&limit=12&sort=position`,
        );
        const data = await response.json();
        const cleanedProjects = data.docs.map((project: CompletedProject) => ({
          ...project,
          slug: project.slug.replace(/^\/+/, ""),
        }));
        setCompletedProjects(cleanedProjects);
      } catch (error) {
    ...
    <>
      ...
      <div className="container mx-auto px-4 pb-12 pt-32">
        <h1 className="mb-8 text-center text-4xl font-bold text-gray-900 dark:text-white">
          {t("completed.title", "Completed Projects")}
        </h1>

        <p className="mb-12 text-center text-lg text-gray-600 dark:text-gray-400">
          {t(
            "completed.description",
            "View our completed projects and achievements.",
          )}
        </p>
        ...
        <div className="mb-12">
          <input
            type="text"
            placeholder={t("completed.searchPlaceholder", "Search projects...")}
            value={searchQuery}
        ...
        {filteredProjects.length === 0 ? (
          <div className="mt-16 flex flex-col items-center">
            ...
            <p className="text-xl text-gray-500">
              {t("completed.noResults", "No completed projects found.")}
            </p>
          </div>
        ) : (
